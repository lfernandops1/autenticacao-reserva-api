...
public class AutenticacaoCadastroServiceImpl implements AutenticacaoCadastroService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AutenticacaoCadastroServiceImpl.class);
    ...
    public void atualizar(AtualizarUsuarioRequest dto) {
        executarComandoComTratamentoErroComMensagem(() -> {
            ...
            if (autenticacaoAntiga == null) {
                throw new UsuarioNaoEncontradoException("Usuário não possui autenticação para atualizar.");
            }

            Autenticacao autenticacaoAtualizada = construirAutenticacaoAtualizada(dto, autenticacaoAntiga);
            ...
            historicoAutenticacaoService.registrarHistoricoCompleto(autenticacaoAntiga, autenticacaoPersistida, usuario);
    ...
    public void desativar(UUID usuarioId) {
        executarComandoComTratamentoErroComMensagem(() -> {
            Autenticacao authAntes = autenticacaoRepository.buscarPorUsuarioId(usuarioId)
            ...
            historicoAutenticacaoService.registrarHistoricoCompleto(authAntes, authDepois, authAntes.getUsuario());
    ...
    private void validarSeAutenticacaoJaExiste(Usuario usuario) {
        autenticacaoRepository.findByUsuario(usuario).ifPresent(auth -> {
    ...
    private Autenticacao construirAutenticacaoAtualizada(AtualizarUsuarioRequest dto, Autenticacao atual) {
